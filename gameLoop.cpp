...
void gameLoop(Window* window, inputHandler* handler) {
	...
	while (!handler->quit) {
		...
		if (level.gameOver) {
			...
			text.display("PRESS ENTER TO RETURN", window->gameRenderer, 85, 400);
			text.display("TO MENU", window->gameRenderer, 225, 425);
			if (handler->keyState[SDL_SCANCODE_RETURN] || handler->keyState[SDL_SCANCODE_SPACE]) {
				handler->inputTimer.start();
				return;
			}
		...
		text.display(player.getScore(), window->gameRenderer, 225, 0); // display player score
